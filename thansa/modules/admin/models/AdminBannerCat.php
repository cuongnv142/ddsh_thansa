...
/**
 * AdminBannerCat represents the model behind the search form about `app\models\BannerCat`.
 */
class AdminBannerCat extends BannerCat {
    ...
    public static function getBannerCatLevel($id = 0, $level = 0, $language = '') {
        $data_option = array();
        $id = (int) $id;
        $level = (int) $level;
        if (!$level && !$id) {
        ...
        for ($i = 0, $n = count($arr); $i < $n; $i++) {
            if ($arr[$i]['level'] == 1) {
                ...
                $id = $arr[$i]['id'];
                unset($arr[$i]);
                $data_option = array_merge($data_option, self::getarr_Bannercat($arr, $id));
            } else {
                break;
            }
        }
    ...
    public static function getarr_Bannercat($options = array(), $id = 0) {
        $data_option = array();
        foreach ($options as $i => $item) {
            if ($item['parent_id'] == (int) $id) {
                ...
                unset($options[$i]);
                $data_option = array_merge($data_option, self::getarr_Bannercat($options, $item['id']));
            }
        }
        return $data_option;
    }
