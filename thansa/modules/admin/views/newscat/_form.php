...
<div class="news-cat-form">
    ...
    if ((int) $model->id) {
        ...
        echo $form->field($model, 'name')->textInput(['maxlength' => 300, 'class' => 'col-xs-10 col-sm-8', 'onkeyup' => 'safetitle(this,"adminnewscat-alias")']);
    }
    echo $form->field($model, 'alias')->textInput(['maxlength' => 300, 'class' => 'col-xs-10 col-sm-8']);
    ?>
    <?= $form->field($model, 'title_seo')->textInput(['maxlength' => 255, 'class' => 'col-xs-10 col-sm-8']) ?>
    <?= $form->field($model, 'content_seo')->textarea(['class' => 'autosize-transition form-control limited', 'maxlength' => 255]) ?>
    <?= $form->field($model, 'key_seo')->textarea(['class' => 'autosize-transition form-control limited', 'maxlength' => 255]) ?>
    <?php
    echo $form->field($model, 'parent_id')->dropDownList(ArrayHelper::map(AdminNewsCat::getNewsCatLevel($model->id, $model->level,$model->language), 'id', 'name'), ['prompt' => 'Danh mục gốc', 'class' => 'width-40 chosen-select']);
    ?>
    <?php
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <span class="profile-picture">
                    <?= Html::img(FileUpload::thumb_wmfile(200, $model->image)) ?>
                </span>
                <label>
    ...
    ?>
    <div class="col-sm-9">
        <textarea class="ckeditor" id="editor_description"
                  name="<?= Html::getInputName($model, 'description') ?>"><?= $model->description ?></textarea>
