...
//            },
        ...
        $('.utilityradius input[type=radio], .social-radio input[type=radio]').bind('change', this, function (evt) {
...
pmdc.prototype.SearchAction = function () {
    ...
    var types = '';
    $('.social-radio input[type=radio]:checked').each(function () {
        if (types.length > 0)
            types += ',';
        types += $(this).val();
    });
        ...
        $('.social-radio input[type=radio]:checked').each(function () {
            ...
            for (var i = 0; i < $this.dataUtilities.length; i++) {
                ...
                if (util.typeid == typeid) {
                    ...
                    html += ' <div class="maps-item">' +
                            '<div class="maps-name">' +
                            '<a href="javascript:void(0);" onclick="showClick(' + i + ')" class="maps-title">' + util.name + '</a>' +
                            '<span class="maps-sub">' + util.address + '</span>' +
                            '</div>' +
                            '</div>';
